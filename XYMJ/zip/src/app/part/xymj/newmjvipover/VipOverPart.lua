...
--激活模块
function VipOverPart:activate(gameId,data,tableid,creatorId,creatorName)
	--激活模块
    self.game_id = gameId
    VipOverPart.super.activate(self, CURRENT_MODULE_NAME)
    self:vipOverDataInfo(data,tableid,creatorId,creatorName)
end


--总分排行
function VipOverPart:vipOverDataInfo(data,tableid,creatorId,creatorName)
    local vip_data = {}
    ...
    self.view:vipOverDataInfo(data,vip_data,tableid,creatorId,creatorName)
end
...
function VipOverPart:deactivate()
    ...
    self.view =  nil
    if self.owner and self.owner.onVipOverPartDeactivate then
        self.owner:onVipOverPartDeactivate()
    end
end
