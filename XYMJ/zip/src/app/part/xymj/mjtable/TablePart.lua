...
function TablePart:onReconnectAck(data,gameId)
	-- body
	self:endLoading()
	local enter_room_ack = game_start_message_pb.StartGameMsgAck()
	...
	local function revertMJRoom()
		self:deactivate()
		self:activate(self.game_id,enter_room_ack)
...
function TablePart:activate(gameId,data)
	...
	local net_mode = global:getModuleWithId(ModuleDef.NET_MOD)
	net_mode:registerMsgListener(SocketConfig.MSG_GAME_OPERATION,handler(self,TablePart.gameOperation))  -- 没有操作 
	...
	net_mode:registerMsgListener(SocketConfig.MSG_PLAYER_OPERATION_NTF,handler(self,TablePart.ntfOperationHandle)) --提醒玩家进行操作
	net_mode:registerMsgListener(SocketConfig.MSG_GAME_START,handler(self,TablePart.gameStartNtf)) --牌局开始
	net_mode:registerMsgListener(SocketConfig.MSG_PLAYER_OPERATION,handler(self,TablePart.playerOperationHandle))
...
function TablePart:activateDefualtPart(data)
	if data and data.tableinfo then
    ...
    if ready_part and not self.record_mode then
    	ready_part:activate(self.game_id, data, data.tableinfo.tablepos)
    end
...
function TablePart:deactivate()
	...
	local readyPart = self:getPart("NormalReadyPart")
	if readyPart then
		readyPart:deactivate()
	end
...
function TablePart:getCard()
	...
	print("###[TablePart:getCard] self.last_card_num is ", self.last_card_num)
	self:updateLastCardNum(self.last_card_num)
end

function TablePart:updateLastCardNum(num)
	-- body
	print("###[TablePart:updateLastCardNum] self.last_card_num is ", self.last_card_num)
...
function TablePart:gameEnd(data)
	...
	if game_end then
		...
		game_end:activate(self.game_id,data , self.m_seat_id,last_round)
...
function TablePart:nextGame()
	-- body
	print("--------------------this is TablePart:nextGame")
    local ready_part = self:getPart("NormalReadyPart")
    if ready_part then
    	ready_part:showView()
    end
...
function TablePart:gameOperationAck(data,appId)
	...
	if game_op_ack.opertaionID == RoomConfig.GAME_OPERATION_TABLE_ADD_NEW_PLAYER then
		...
		local tablePos = game_op_ack.tablePos
		local ready_part = self:getPart("NormalReadyPart")
...
function TablePart:ntfOperationHandle(data,appId)
	local ntf_operation = ycmj_message_pb.PlayerOperationNotifyMsg()
	ntf_operation:ParseFromString(data)
	print(string.format("###[TablePart:ntfOperationHandle]  0x%02x", ntf_operation.operation))
	self:ntfOperation(ntf_operation, appId)
end


function TablePart:ntfOperation(ntf_operation,appId)
	-- body
	...
	elseif ntf_operation.operation == RoomConfig.MAHJONG_OPERTAION_REMOE_CHU_CARD then --玩家打出的牌，被吃碰杠走了
		print("###[TablePart:ntfOperation]玩家打出的牌，被吃碰杠走了")
	...
	elseif ntf_operation.operation == RoomConfig.MAHJONG_OPERTAION_HU then
		print("###[TablePart:ntfOperation]胡 type ", ntf_operation.hutype)
		self:operationCard(ntf_operation)
	elseif bit._and(ntf_operation.operation,RoomConfig.MAHJONG_OPERTAION_CHU) == RoomConfig.MAHJONG_OPERTAION_CHU then --轮到玩家出牌
		print("###[TablePart:ntfOperation]RoomConfig.MAHJONG_OPERTAION_CHU 轮到玩家出牌")
	...
	elseif ntf_operation.operation == RoomConfig.MAHJONG_OPERTAION_TIP then --提示当前谁在操作
		print("###[TablePart:ntfOperation]提示当前谁在操作")
...
function TablePart:operationHu(data)
	...
	local function checkMa(i)
		...
		if temp > 0 then
			...
			card_part:showHuAnimate(view_id,m_table,data.hutype)
...
function TablePart:updateHuCard(data)
	...
	if data.tinglist and #(data.tinglist) > 0 then
    	...
    	print("###[TablePart:updateHuCard] data.tinglist and #(data.tinglist) > 0 self.last_card_num is ", self.last_card_num)
...
function TablePart:operationTip(data)
	-- body
	self.last_card_num = data.cardleftnum
	print("###[TablePart:operationTip] self.last_card_num is ", self.last_card_num)
	self.view:updateLastCardNum(self.last_card_num)
	local card_part = self:getPart("NormalCardPart")
	if card_part then
...
function TablePart:addOutCard(data)
	-- body
	self.last_card_num = data.cardleftnum
	print("###[TablePart:addOutCard] self.last_card_num is ", self.last_card_num)
	self.view:updateLastCardNum(self.last_card_num)
...
function TablePart:operationCard(data)
	...
	if bit._check(data.operation, RoomConfig.PIZI_NOTIFY) then
		self:getPart("CardPart"):addOpt(RoomConfig.PIZI_NOTIFY)
		dis_play_guo = false
	end 

	if bit._and(data.operation,RoomConfig.Gang) == RoomConfig.Gang  then
  		print("###[TablePart:operationCard] card_part:ntfGangList2")
...
function TablePart:gameStartNtf(data,appId)
    ...
    if chat_part then
		...
		chat_part:setTablePos(pos_table)
		chat_part:setChatBtnStat(true)
    end
end
...
function TablePart:playerOperationHandle(data,appId)
...
function TablePart:playerOperation(player_operaction,appId)
	-- body
	local card_part = self:getPart("NormalCardPart")

	local cur_seat_id = player_operaction.player_table_pos
	local cur_view_id = self:changeSeatToView(cur_seat_id)
	print(string.format("###[TablePart:playerOperation] 操作回包 %x", player_operaction.operation))

	if player_operaction.operation == RoomConfig.MAHJONG_OPERTAION_POP_LAST then --刷新左上角的尾牌
		print("###[TablePart:playerOperation] 刷新左上角的尾牌回包")
		local cur_seat_id = player_operaction.player_table_pos
		local cur_view_id = self:changeSeatToView(cur_seat_id)
	...
	elseif player_operaction.operation == RoomConfig.MAHJONG_OPERTAION_CHU then --其他玩家出牌
		print("###[TablePart:playerOperation] 玩家出牌回包")
		...
		if nil ~= card_part.isLaiZi and true == card_part:isLaiZi(card_value) then
			card_part:playChiPengGangAni(cur_view_id, RoomConfig.LaiziGang, false) 
		end
	elseif player_operaction.operation == RoomConfig.MAHJONG_OPERTAION_AN_GANG
        ...
        print("###[TablePart:playerOperation] 点杠操作回包 ", player_operaction.operation)
        if player_operaction.opValue == RoomConfig.MAHJONG_OPERTAION_GANG_NOTIFY  then
        	print("###[TablePart:playerOperation] player_operaction.opValue == RoomConfig.MAHJONG_OPERTAION_GANG_NOTIFY")
         	return
        end 

        print(string.format("###[TablePart:playerOperation] 点杠操作回包 %02x, %02d", player_operaction.operation, player_operaction.opValue))
        self:doGangShowLogic(cur_seat_id, player_operaction.operation, player_operaction.opValue)

	elseif player_operaction.operation == RoomConfig.MAHJONG_OPERTAION_PENG then --碰
		print("###[TablePart:playerOperation] 点碰操作回包")
	...
	elseif player_operaction.operation == RoomConfig.MAHJONG_OPERTAION_CHI then --吃
		print("###[TablePart:playerOperation] 点吃操作回包")
		local opvalue = player_operaction.opValue
		local c = {}
		for i=1,4 do
...
function TablePart:getSrcGangCard(card_value)
	...
	elseif c3 == c4 then
      ...
      op_card = bit._or(card_value,0xffffff)
    end
...
function TablePart:doGangShowLogic(seatID, gangType, card_value)  
	...
	if bit._and(gangType,RoomConfig.MAHJONG_OPERTAION_AN_GANG) == RoomConfig.MAHJONG_OPERTAION_AN_GANG then
		...
		if seatID == self.m_seat_id then --我自己可以看见第二张牌
			card_data = {mcard= {RoomConfig.EmptyCard,RoomConfig.EmptyCard,RoomConfig.EmptyCard,outCard}}
		end
