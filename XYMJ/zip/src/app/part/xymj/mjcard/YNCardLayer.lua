...
local YNCardLayer = class("YNCardLayer",CardLayer)

--云南杠牌处理
function YNCardLayer:GangPicClick()
	-- body
...
--杠的同牌不同组合 的选择列表
function YNCardLayer:showGangList(gangList)
	--显示可以杠的列表
...
--杠的不同牌 单个选择列表
function YNCardLayer:showGangSelect(gangList)
	-- body
...
--显示碰杠过操作
function YNCardLayer:showOpt(type,value)
	-- body
...
function YNCardLayer:showHuAnimate(viewId,maList)
	-- body
	local card_node = self.node["hcard_node" .. viewId]
	local pos = cc.p(card_node:getPosition())
	-- local content_size = self.node.hu_sprite:getContentSize()
	-- if viewId == RoomConfig.MySeat then
	-- 	pos = cc.pAdd(pos,cc.p(content_size.width,content_size.height))
	-- elseif viewId == RoomConfig.UpSeat then
	-- 	pos = cc.pAdd(pos,cc.p(content_size.width/2,0))
	-- elseif viewId == RoomConfig.DownSeat then
	-- 	pos = cc.pAdd(pos,cc.p(-content_size.width/2,0))
	-- elseif viewId == RoomConfig.FrontSeat then
	-- 	pos = cc.pAdd(pos,cc.p(content_size.width,-content_size.height/2))
	-- end
