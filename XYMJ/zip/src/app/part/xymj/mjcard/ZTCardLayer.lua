...
function ZTCardLayer:showHuAnimate(viewId,maList)
	-- body
...
function ZTCardLayer:baipaiShowCardsView(viewId, huList)
	...
	for i,v in ipairs(huList) do
		...
		print("ZTCardLayer:baipaiShowCardsView--index=",i,",value=",v)
		
		if viewId == RoomConfig.MySeat then
			pos = cc.p((i-1)*content_size.width, 0)
		elseif viewId == RoomConfig.DownSeat then
			zOrder = baipaiCount - i
			pos = cc.p(0, (i-1)*content_size.height*self.OutCardSortOffsetCol)
		elseif viewId == RoomConfig.FrontSeat then
			pos = cc.p((i-1)*content_size.width*card:getScale(),0)
		...
		baipaiNode:addChild(card, zOrder)
	end
end
