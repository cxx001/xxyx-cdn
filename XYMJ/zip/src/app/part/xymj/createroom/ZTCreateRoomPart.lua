...
local ZTCreateRoomPart = class("ZTCreateRoomPart",CreateRoomPart) --创建房间

function ZTCreateRoomPart:createGame()
	-- body
	local net_mode = global:getModuleWithId(ModuleDef.NET_MOD)
	local create_vip_room = wllobby_message_pb.CreateVipRoomMsg()
	create_vip_room.roomid = 2012 --vip场，个旧的roomid
	create_vip_room.gametype = 1

	local quanNum = CreateRoomPart["TIMAES"..self.cur_times]
	print("------------quanNum_ZTMJ : ",quanNum)
	...
	create_vip_room.selectWayNum = RoomConfig.PlayRule[3]
