...
function ReadyPart:activate(gameId,data,mySeatID)
 ...
 --    end
	if data ~= nil then 
		...
		self:updateTableInfo(self.m_tableInfo)
	end
	self.isReady = false
...
function ReadyPart:updateTableInfo(info)
	self.view:updateTableShow(info)
end
...
function ReadyPart:registNetMsg(isReg)
	...
	if isReg then
		...
		net_mode:registerMsgListener(ReadyPart.CMD.MSG_GET_GAME_CONFIG_RSP,handler(self,ReadyPart.onGetConfigFromServer), partId)
	else
		net_mode:unRegisterMsgListener(ReadyPart.CMD.MSG_PLAYER_OPERATION, partId)
		net_mode:unRegisterMsgListener(ReadyPart.CMD.MSG_GET_GAME_CONFIG_RSP, partId)
	end
end
...
function ReadyPart:addPlayer(playerInfo) 
	-- body
	if self.owner.match_mode then
		self.player_list = self.owner.player_list
		self.m_pos 		 = self.owner.m_seat_id
	end
...
function ReadyPart:doUpdatePlayer(infos, myViewID)
	...
	for k1,playerInfo in pairs(self.player_list) do
		...
		if nil ~= stateInfo then
			...
			if playerInfo.view_id == myViewID and self.m_tableInfo.currenthand <= 0 then
...
function ReadyPart:hideReadyBtn()
	self.view:updateVIPOpBtn(false)
end

function ReadyPart:getConfigFromServer()
	local net_mode = global:getModuleWithId(ModuleDef.NET_MOD)
	local get_config_req = get_config_message_pb.GetAnnoucementReq()
	net_mode:sendProtoMsg(get_config_req,ReadyPart.CMD.MSG_GET_GAME_CONFIG_REQ,self.game_id)
end

function ReadyPart:onGetConfigFromServer(data,appId)
...
function ReadyPart:isFirstRound()
	local ret = true
	if self.m_tableInfo and self.m_tableInfo.currenthand and self.m_tableInfo.currenthand > 0 then
		ret = false
	end
	return ret
end
